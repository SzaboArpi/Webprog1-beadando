...
--
-- Gép: 127.0.0.1
-- Létrehozás ideje: 2026. Ápr 18. 20:18
...
CREATE TABLE `szalloda` (
  ...
  `besorolas` int(1) DEFAULT NULL,
  `helyseg_az` int(1) DEFAULT NULL,
  `tengerpart_tav` int(3) DEFAULT NULL,
  `repter_tav` int(2) DEFAULT NULL,
  `felpanzio` int(1) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
...
CREATE TABLE `tavasz` (
  ...
  `indulas` varchar(10) DEFAULT NULL,
  `idotartam` int(2) DEFAULT NULL,
...
ALTER TABLE `szalloda`
  ...
  ADD KEY `helyseg_az_2` (`helyseg_az`);
...
--
ALTER TABLE `szalloda`
  ADD CONSTRAINT `szalloda_ibfk_1` FOREIGN KEY (`helyseg_az`) REFERENCES `helyseg` (`az`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Megkötések a táblához `tavasz`
--
ALTER TABLE `tavasz`
  ADD CONSTRAINT `tavasz_ibfk_1` FOREIGN KEY (`szalloda_az`) REFERENCES `szalloda` (`az`) ON DELETE NO ACTION ON UPDATE NO ACTION;
COMMIT;
